class PlayerSphere {
    ...
    draw(cosmetics = {}) {
        ...
        // Foreground pass (in front of sphere)
        if (effect !== 'none') {
            this.drawEffectForeground(effect);
        }
    }
    ...
    drawHat(hat) {
        if (hat === 'crown') {
            ...
            this.ctx.lineWidth = this.radius * 0.06;
            this.ctx.fillRect(this.centerX - this.radius * 0.5, this.centerY - this.radius * 1.16, this.radius, this.radius * 0.26);
            this.ctx.strokeRect(this.centerX - this.radius * 0.5, this.centerY - this.radius * 1.16, this.radius, this.radius * 0.26);
            this.ctx.fillRect(this.centerX - this.radius * 0.66, this.centerY - this.radius * 0.9, this.radius * 1.33, this.radius * 0.16);
            this.ctx.strokeRect(this.centerX - this.radius * 0.66, this.centerY - this.radius * 0.9, this.radius * 1.33, this.radius * 0.16);
            ...
            this.ctx.beginPath();
            this.ctx.moveTo(this.centerX, this.centerY - this.radius * 1.5);
            this.ctx.lineTo(this.centerX - this.radius * 0.5, this.centerY - this.radius * 0.9);
            ...
            const gradient = this.ctx.createLinearGradient(
                this.centerX - this.radius * 0.4, this.centerY - this.radius * 1.6,
                this.centerX + this.radius * 0.4, this.centerY - this.radius * 0.9
            );
            ...
            this.ctx.fillStyle = gradient;
            this.ctx.beginPath();
            this.ctx.moveTo(this.centerX, this.centerY - this.radius * 1.6);
    ...
    // ─── BACKGROUND PASS (renders behind sphere) ───────────────────────────────
    drawEffect(effect) {
        const t = Date.now() / 1000;
        const cx = this.centerX, cy = this.centerY, r = this.radius;

        if (effect === 'mystical-aura') {
            const pulseSize = r * (1.4 + Math.sin(t * 2) * 0.1);
            const gradient1 = this.ctx.createRadialGradient(cx, cy, r * 0.9, cx, cy, pulseSize);
            gradient1.addColorStop(0, 'rgba(0, 140, 153, 0.3)');
            gradient1.addColorStop(1, 'transparent');
            this.ctx.fillStyle = gradient1;
            this.ctx.beginPath();
            this.ctx.arc(cx, cy, pulseSize, 0, Math.PI * 2);
            this.ctx.fill();

        } else if (effect === 'golden-aura') {
            const pulseSize = r * (1.5 + Math.sin(t * 3) * 0.15);
            const gradient = this.ctx.createRadialGradient(cx, cy, r * 0.8, cx, cy, pulseSize);
            ...
            const pulseSize = r * (1.65 + Math.sin(t * 3.5) * 0.14);
            const gradient = this.ctx.createRadialGradient(cx, cy, r * 0.7, cx, cy, pulseSize);
            ...
            for (let i = 0; i < 7; i++) {
                ...
                this.ctx.strokeStyle = `rgba(180, 80, 255, ${0.5 + Math.sin(t * 3.5 + i) * 0.3})`;
            ...
            // Bicolor aura glow only (behind sphere)
            ...
            if (this.wingImageLoaded) {
                const wingWidth = r * 2.4;
                const wingHeight = r * 3.0;
                const flapOffset = Math.sin(t * 3) * 0.15;
                this.ctx.save();
                this.ctx.translate(cx - r * 0.7, cy);
                this.ctx.rotate(-flapOffset);
                this.ctx.drawImage(this.wingImage, -wingWidth, -wingHeight / 2, wingWidth, wingHeight);
                this.ctx.restore();
                this.ctx.save();
                this.ctx.translate(cx + r * 0.7, cy);
                this.ctx.rotate(flapOffset);
            ...
            if (!this._solar) {
                this._solar = { particles: [], loops: [], flares: [], flareTimer: 0, lastTime: t };
                const s = this._solar;
                ...
                class CoronalLoop {
                    ...
                    draw(ctx) {
                        ...
                        ctx.save();
                        ctx.strokeStyle = this.color === 'orange' ? `rgba(255,140,0,${alpha * 0.45})` : `rgba(255,220,120,${alpha * 0.55})`;
                        ...
                        ctx.strokeStyle = this.color === 'orange' ? `rgba(255,230,100,${alpha * 0.9})` : `rgba(255,255,255,${alpha * 0.95})`;
    ...
    // ─── FOREGROUND PASS (renders in front of sphere) ──────────────────────────
    drawEffectForeground(effect) {
        const t = Date.now() / 1000;
        const cx = this.centerX, cy = this.centerY, r = this.radius;
        ...
        if (effect === 'mystical-aura') {
            ...
            for (let i = 0; i < 10; i++) {
                const angle = (t * 2.2) + (i * Math.PI / 5);
                const x = cx + Math.cos(angle) * r * 1.55;
                const y = cy + Math.sin(angle) * r * 1.55;
                this.ctx.save();
                this.ctx.translate(x, y);
                this.ctx.rotate(t * 3);
                this.ctx.fillStyle = i % 2 === 0 ? '#ff9d00' : '#8a4dff';
                this.ctx.font = `${r * 0.24}px Arial`;
                this.ctx.textAlign = 'center';
                this.ctx.textBaseline = 'middle';
                this.ctx.fillText('✦', 0, 0);
                this.ctx.restore();
            ...
            if (!this._solar) return;
            const s = this._solar;
            const ctx2 = this.ctx;

            // Surface granules on top of sphere
            s.particles.filter(p => p.type === 'granule').forEach(p => p.draw(ctx2));

            // Flare eruptions
            s.flareTimer -= (this._solar._lastFgDt || 0);
            this._solar._lastFgDt = Math.min(t - (this._solar._lastFgT || t), 0.05);
            this._solar._lastFgT = t;
            ...
            const dt = this._solar._lastFgDt;
            for (let i = s.flares.length - 1; i >= 0; i--) {
                ...
                const bx = cx + Math.cos(f.angle) * r * 0.92;
                const by = cy + Math.sin(f.angle) * r * 0.92;
            ...
            limb.addColorStop(1, 'rgba(255,60,0,0.35)');
    ...
    drawSword(sword) {
        ...
        const swordX = this.centerX + this.radius * 1.2;
